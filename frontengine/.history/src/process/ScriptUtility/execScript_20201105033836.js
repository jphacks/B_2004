...
function execScript (body, array) {
  ...
  const error = []
  // 引数をとる
  const row = Object.keys(body.body.body).length
  console.log(row)
  // console.log(Object.values(body.body.body.type))
  console.log(body.body.body[0].type)
  ...
  for (let i = 0; i < row; i++) {
    switch (body.body.body[i].type) {
      // 宣言
      case 'VariableDeclaration':
        for (const decalate of body.body.body[i].decrations) {
          if (local[decalate.id.name]) {
            error.push(decalate)
            continue
          }
          local[decalate.id.name] = decalate.init ? CheckProperty(decalate.init) : null
          localInfo[decalate.id.name] = body.body.body[i].kind
        }
        // console.log('てすうううううううううううと')
        break
      case 'ExpressionStatement':
        if (body.body.body[i].expression && body.body.body[i].expression.type === 'CallExpression') {
          const kumikomi = local[body.body.body[i].expression.callee.object]
          const target = body.body.body[i].expression.callee.object
          const arg = body.body.body[i].expression.arguments
          // ex: hairetu = []
          // hairetu = [ ...[1,2,3,4,5]]
          let kumikomiFuncName = ''
          if(body.body.body[i].expression.callee &&  body.body.body[i].expression.callee.property) {
            kumikomiFuncName = body.body.body[i].expression.callee.property.name
          }
          if (body.body.body[i].expression.arguments.type === 'ArrowFunctionExpression') {
            const recName = body.body.body[i].expression.arguments.body
